...
EOF
    ...
    fix_failed_fetch
    sudo rm -rf "$HOME/.aztec/testnet/data" && sudo mkdir -p "$HOME/.aztec/testnet" && sudo wget https://files5.blacknodes.net/aztec/aztec-testnet.tar.lz4 -O /root/aztec-testnet.tar.lz4 && sudo lz4 -d /root/aztec-testnet.tar.lz4 | sudo tar x -C "$HOME/.aztec/testnet" && sudo rm /root/aztec-testnet.tar.lz4 && sudo chown -R "$USER":"$USER" "$HOME/.aztec/testnet" && sudo systemctl restart aztec
}
...
update_node() {
    echo -e "${YELLOW}🔄 Updating Aztec Node to 2.0.2...${NC}"
    sudo systemctl stop aztec
